...
- `export const revalidate = 300` on any route that reads Payload. Five minutes — long
  ...
  publish sees the change while still looking, and short enough that a post scheduled with
  a future `publishedAt` appears soon after its date passes. Revalidation is lazy, so an
  unread route costs nothing.
- A route with a dynamic segment gets `generateStaticParams`. Return **only that segment's
  params** — `[locale]` is generated by the layout above. Apply the same visibility filter
  the page applies (`publishedAt <= now` for news): a prerendered page is a file written at
  build time, so anything scheduled must be left out and picked up by `dynamicParams`,
  which re-checks against the time of the request and still 404s until the date passes.
